...
export default async function LoginPage({
  ...
  return (
    <main
      className={`${display.variable} ${body.variable} relative flex min-h-screen items-center justify-center overflow-hidden bg-(--color-login-ink) px-4 py-10 font-(family-name:--font-manrope)`}
    >
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(110%_80%_at_50%_-15%,rgba(255,221,69,.18),transparent_62%)]"
      />
      <div
        aria-hidden
      ...
      <div className="relative w-full max-w-[400px] rounded-[18px] bg-white px-[34px] pt-9 pb-[26px] shadow-[0_24px_50px_-22px_rgba(0,0,0,.6)]">
        ...
        <div className="mb-[26px] flex flex-col items-center gap-3.5">
          <div className="flex h-[62px] w-[62px] items-center justify-center rounded-2xl bg-(--color-login-ink) shadow-[inset_0_-3px_0_var(--color-login-yellow)]">
            <Lodjur className="h-9 w-9 brightness-0 invert" />
          </div>
          <div className="flex flex-col items-center gap-[5px]">
            <h1 className="font-(family-name:--font-space-grotesk) text-2xl font-semibold tracking-[-0.01em] text-(--color-login-ink)">
              Logga in på Schema
            </h1>
        ...
        <div className="mt-[22px] flex items-center justify-between border-t border-(--color-login-rule) pt-4 text-xs text-(--color-login-foot)">
          ...
          <span className="flex items-center gap-1.5">
            <span
              aria-hidden
              className="h-1.5 w-1.5 rounded-full bg-(--color-login-yellow)"
            />
            Support 0481-423 00
          </span>
        </div>
